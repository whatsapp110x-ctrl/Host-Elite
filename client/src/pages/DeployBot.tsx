...
export default function DeployBot() {
  ...
  const autoDetectMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await apiRequest('POST', '/api/analyze-zip', formData);
      return response.json();
    },
    onSuccess: (analysis) => {
      setAutoAnalysis(analysis);
      if (analysis.suggestedCommand) {
        form.setValue('runCommand', analysis.suggestedCommand);
        if (analysis.language) {
          form.setValue('language', analysis.language);
        }
        toast({
          title: 'Auto-detection successful!',
          description: `Detected ${analysis.language || 'unknown'} project with command: ${analysis.suggestedCommand}`,
        });
      } else {
        toast({
          title: 'Auto-detection complete',
          description: 'Could not determine run command automatically. Please set it manually.',
          variant: 'destructive',
        });
      }
    },
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <label className="block text-slate-300 font-medium">Run Command *</label>
                  {deploymentMethod === 'zip' && selectedFile && (
                    <button
                      type="button"
                      onClick={() => {
                        setAutoDetecting(true);
                        autoDetectMutation.mutate(selectedFile);
                      }}
                      disabled={autoDetectMutation.isPending}
                      className="bg-gradient-to-r from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 disabled:from-slate-600 disabled:to-slate-700 text-white px-3 py-1 rounded-md text-sm font-medium transition-all duration-200 flex items-center"
                ...
                <div className="flex items-start justify-between mt-1">
                  <p className="text-xs text-slate-400">Command to start your bot</p>
                  {autoAnalysis && (
                    <div className="text-xs text-green-400">
                      <i className="fas fa-check-circle mr-1"></i>
                      Auto-detected from {autoAnalysis.hasProcfile ? 'Procfile' : autoAnalysis.hasDockerfile ? 'Dockerfile' : 'project structure'}
                    </div>
                  )}
                </div>
              </div>
