...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (botsError) {
      ...
      toast({
        ...
        description: 'Unable to fetch bot data. Check your connection.',
        variant: 'destructive',
      });
    }
  }, [botsError, toast]);

  const { data: statsData, error: statsError } = useQuery<BotStats>({
    queryKey: ['/api/stats'],
    refetchInterval: 5000,
  });
  ...
  useEffect(() => {
    if (statsData && typeof statsData === 'object' && 'totalBots' in statsData) {
      setStats(statsData as BotStats);
    }
  }, [statsData]);
